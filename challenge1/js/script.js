...
function matchCountries() {
	...
	if (country.length > 0) {
	    ...
	    for (let i = 0; i < countries.length; i++){
			let letters = country.length;
			let match = countries[i].toLowerCase().substring(0, letters);
			if (match == country.toLowerCase()) {
			con.push(countries[i]);
			}
	    }
	    	...
	    	let arr1 =[];
	   		 for (let k = 0; k < con.length; k++){
	   		 	arr1[k] = document.createElement('li');
	   		 	arr1[k].classList.add('select');
	   		 	arr1[k].textContent = con[k];
	   		 	list.appendChild(arr1[k]);
	   		 	let result = document.getElementsByClassName('select');
	   			 result[k].addEventListener('click', () => {
	    				document.getElementById("country").value = result[k].innerHTML;
	    		 });
	    } 
